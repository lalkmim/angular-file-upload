        ...
        link: function(scope, element, attrs) {
            $(element).fileupload({
                ...
                formData: [{
                    ...
                    var queue = $(element).find('#queue');
                    $('<div />').text(data.result.name).appendTo(queue);
                    
                    var video = $(element).find('#video');
                    video.html('');
                    
                    $('<div class="wistia_embed wistia_async_' + data.result.hashed_id + '" style="height:349px;width:620px">&nbsp;</div>').appendTo(video);
